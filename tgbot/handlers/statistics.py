    ...
    if works_list:
        ...
        for index, stats_el in enumerate(stats_list):
            text_to_send += (f"{index + 1}. <b>{stats_el['general']['name']} <a href='https://crodconnect.ru/stats?uuid={None}&tid={message.from_user.id}&work={stats_el['general']['work_id']}'>#{stats_el['general']['work_id']}</a></b> | "
                             f"📑 {stats_el['results']['final_mark']}/{stats_el['results']['max_mark']} | "
                             f"⌛ {stats_el['general']['time']['end'] - stats_el['general']['time']['start']}")

        text_to_send += "\n\n<b>Для того, чтобы посмотреть подробную статистику по решённому варианту, нажми на номер задания, который обозначен символом <code>#</code></b>"
